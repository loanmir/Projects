...
public class GameWindow extends JFrame implements ActionListener{
    ...
    class StyledButton extends BasicButtonUI{
        @Override
        public void installUI (JComponent c) { // Method called when UI is installed on component
            super.installUI(c);
            AbstractButton button = (AbstractButton) c;
            button.setOpaque(false); // Background not painted by default UI -> paint()
            button.setBorder(new EmptyBorder(5, 15, 5, 15));

        }

        @Override
        public void paint (Graphics g, JComponent c) {  // Method overrides the default button painting behaviour
